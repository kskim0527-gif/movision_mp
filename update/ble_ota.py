...
async def send_file(client, data, cmd_begin, cmd_end, label, chunk_size):
    ...
    # ── 파티션 지우기 대기 (고정값) ──────────────────────────────
    # ESP32 플래시 지우기: 4KB 블록당 ~10ms → 4MB = ~10초
    ...
    # SYNC 주기: 100개 패킷마다 (chunk=244 → ~24KB마다)
    SYNC_EVERY = 100
        ...
        try:
            async with BleakClient(address, timeout=20.0) as client:
                ...
                if do_app and do_storage:
                    ...
                    print("  파티션 지우는 중... (4초 대기)", flush=True)
                    ...
                    print("  파티션 지우는 중... (12초 대기)", flush=True)
...
def check_files():
    ...
    print("\n업데이트 파일 확인:")
    if has_app:
        print(f"  ✅ 펌웨어  (movision.bin) : {os.path.getsize(APP_BIN)//1024} KB")
    else:
