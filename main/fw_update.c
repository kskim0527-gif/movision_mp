...
static void fw_start_task(void *pvParameters) {
  ESP_LOGI(TAG, "FW Storage Erasing Task Starting (Size=%lu)...",
           (unsigned long)s_fw_ctx.total_size);
  ...
  esp_err_t err = esp_ota_begin(s_fw_ctx.update_partition, s_fw_ctx.total_size,
                                &s_fw_ctx.update_handle);
...
static void fw_finish_task(void *pvParameters) {
  ...
  ESP_LOGI(TAG,
           "CRC Check Skipped (Exp: 0x%04X, Calc: 0x%04X). Proceeding with "
           "size match.",
...
static void handle_fw_info(const uint8_t *data, size_t len) {
  if (len < 18)
    return;
  // 패킷 규격(v1.10):
  // [H:1][ID:1][CMD:1][LEN:2][Version:6][Size:4][Type:1][CRC:2][T:1]
  ...
  ESP_LOGI(TAG, "FW Info: Size=%lu, Type=%u",
           (unsigned long)s_fw_ctx.total_size, update_type);
...
static void handle_fw_data(const uint8_t *data, size_t len) {
  ...
  // 누적 CRC 계산
  s_fw_ctx.current_crc =
      crc16_ccitt(&data[7], payload_len, s_fw_ctx.current_crc);
  ...
  if (err == ESP_OK) {
    ...
    if (seq % 5 == 4) {
      ...
      // 저장 및 ACK 송신 로그
      if (seq % 20 == 19) {
        ESP_LOGW("BLE_ONLY", "[FW 저장 ACK] Seq=%u, Size=%lu/%lu", seq,
                 (unsigned long)s_fw_ctx.current_size,
                 (unsigned long)s_fw_ctx.total_size);
      }
    }
    ...
    if (s_fw_ctx.current_size >= s_fw_ctx.total_size) {
      // 마지막 블록 수신 로그 출력
      ESP_LOGW("BLE_ONLY", "[FW 저장 완료] Seq=%u, Size=%lu/%lu [FINAL]", seq,
               (unsigned long)s_fw_ctx.current_size,
               (unsigned long)s_fw_ctx.total_size);
    }
...
void process_fw_update_command(const uint8_t *data, size_t len) {
  if (len < 5)
    return;
  if (data[0] != PROTOCOL_HEADER || data[1] != PROTOCOL_ID_FW)
    return;
  ...
  case CMD_FW_DATA:
    ...
    break;
  case CMD_FW_CANCEL:
    ESP_LOGW(TAG, "Update Cancelled by APP. Reason: %u",
             (len > 5) ? data[5] : 0);
